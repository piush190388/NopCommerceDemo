    ...
    public partial class CustomerController : BasePublicController
    {
        ...
        public virtual IActionResult Login(LoginModel model, string returnUrl, bool captchaValid)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                postParameters.Add("client_id", model.Email);
                postParameters.Add("client_secret", "secret");
                postParameters.Add("scope", "apiscope");
                ...
                try
                {
                    HttpWebRequest myHttpWebRequest = (HttpWebRequest)HttpWebRequest.Create("http://localhost:8071/connect/token");
                    ...
                    AccessToken json = JsonConvert.DeserializeObject<AccessToken>(pageContent);
                    string accesstoken = json.access_token;
                    if (accesstoken != null)
                    {
                        Customer _customer = _customerService.GetCustomerByEmail(model.Email);
                        if (_customer == null)
                        {
                            var customer1 = _workContext.CurrentCustomer;
                            customer1.RegisteredInStoreId = _storeContext.CurrentStore.Id;
                            ...
                            var registrationRequest = new CustomerRegistrationRequest(customer1,
                                   model1.Email,
                                   _customerSettings.UsernamesEnabled ? model1.Username : model1.Email,
                                   model1.Password,
